...
export class JobsRepo {
  ...
  create(job: PublishJob): PublishJob {
    this.db
      .prepare(
        `INSERT INTO publish_jobs (id, orgId, postId, connectionId, platform, status, attempts, externalId, externalUrl, error, log, startedAt, finishedAt, createdAt)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(
        job.id,
  ...
  save(job: PublishJob): PublishJob {
    this.db
      .prepare(
        `UPDATE publish_jobs SET status=?, attempts=?, externalId=?, externalUrl=?, error=?, log=?, startedAt=?, finishedAt=? WHERE id=?`
      )
      .run(
        job.status,
        job.attempts,
