...
/** Organization avatar: the uploaded logo when present, otherwise initials on the brand colour. */
export function OrgLogo({ org, size = 32, className, rounded = "rounded-lg" }: { org: Pick<Organization, "name" | "brandColor" | "logoUrl">; size?: number; className?: string; rounded?: string }) {
  if (org.logoUrl) {
    return (
      <span className={cn("inline-flex shrink-0 items-center justify-center overflow-hidden border border-ink-200 bg-glass", rounded, className)} style={{ width: size, height: size }} data-testid="org-logo">
