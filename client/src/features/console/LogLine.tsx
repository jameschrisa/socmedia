...
export function LogLine({ entry }: { entry: LogEntry }) {
  return (
    <div className="term-line flex flex-wrap items-baseline gap-x-2 px-3 py-0.5 text-[12.5px] leading-relaxed">
...
export function SimpleLine({ prefix, text, tone = "default" }: { prefix?: string; text: string; tone?: "default" | "error" | "muted" }) {
  ...
  return (
    <div className="term-line px-3 py-0.5 text-[12.5px] leading-relaxed">
      {prefix && <span className="mr-2 text-brand-300">{prefix}</span>}
      <span className={`whitespace-pre-wrap ${toneClass}`}><Linkified text={text} /></span>
    </div>
  );
}
